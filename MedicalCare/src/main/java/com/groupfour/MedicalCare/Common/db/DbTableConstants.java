package com.groupfour.MedicalCare.Common.db;

public final class DbTableConstants {

    public static final String PACIJENT = "pacijent";
    public static final String ADMINKLINICKOGCENTRA = "admin_kc";
    public static final String REGISTRACIJAPACIJENATA = "registracija_pacijenata";
    public static final String KLINIKA = "klinika";
    public static final String LEKAR = "lekar";
    public static final String MEDICINSKA_SESTRA = "medicinska_sestra";
    public static final String SALA = "sala";
    public static final String SIFARNIK_LEKOVA = "sifarnik_lekova";
    public static final String SIFARNIK_DIJAGNOZA = "sifarnik_dijagnoza";
    public static final String ADMIN_KLINIKE = "admin_klinike";
    public static final String LEKAR_PACIJENT = "lekar_pacijent";
    public static final String MED_SESTRA_PACIJENT = "med_sestra_pacijent";
    public static final String IZVESTAJ_O_PREGLEDU = "izvestaj_o_pregledu";
    public static final String RECEPT = "recept";
    public static final String OPERACIJA = "operacija";
    public static final String ODSUSTVO = "odsustvo";
    public static final String TIP_PREGLEDA = "tip_pregleda";
    public static final String PREGLED = "pregled";
    public static final String OCENA_LEKARA = "ocena_lekara";
    public static final String OCENA_KLINIKE = "ocena_klinike";
    public static final String KARTON = "karton";
    public static final String LEKAR_OPERACIJA = "lekar_operacija";
    public static final String LEKAR_PREGLED = "lekar_pregled";
    public static final String PACIJENT_KLINIKA = "pacijent_klinika";
    public static final String KARTON_IZVESTAJI = "karton_izvestaji";
    public static final String KARTON_DIJAGNOZE = "karton_dijagnoze";
    public static final String IZVESTAJ_RECEPT = "izvestaj_recept";
    public static final String LEKAR_ODSUSTVA = "lekar_odsustva";
    public static final String MED_SESTRA_ODSUSTVA = "med_sestra_odsusutva";

    private DbTableConstants(){

    }
}
